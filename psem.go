...
Two operations can be performed on semaphores: increment the semaphore value by one Post()
...
// If Creat is specified, and a semaphore with the given name already exists,
// then mode and value are ignored.
// flag: Creat|Excl
// mode: for example 0600
// value: initial value
func Open(name string, flag int, mode int, value uint) (*Sem, error) {
	sem := &Sem{}
	_, err := C.Open(&sem.semt, C.CString(name), C.int(flag), C.mode_t(mode), C.uint(value))
	if sem.semt != nil {
		err = nil
	}
	if err != nil {
...
// Post -  increments (unlocks) the semaphore.
// If the semaphore's value consequently becomes greater than zero,
// then another process or thread blocked in a psem.Wait call will be woken up
// and proceed to lock the semaphore.
func (sem *Sem) Post() error {
	_, err := C.Post(sem.semt)
	return err
}

// Wait - decrements (locks) the semaphore.
...
func (sem *Sem) Wait() error {
	_, err := C.Wait(sem.semt)
	return err
}

// TryWait - is the same as Wait(), except that if the decrement cannot be immediately performed,
// then call returns an error instead of blocking.
func (sem *Sem) TryWait() error {
	_, err := C.TryWait(sem.semt)
	return err
}

// TimedWait - is the same as Wait(), except that timeout specifies a limit on the amount of
...
// could not be locked immediately, then TimedWait() fails with a timeout error.
// If the operation can be performed immediately,
// then TimedWait() never fails with a timeout error, regardless of the value of timeout.
// Furthermore, the validity of timeout is not checked in this case.
func (sem *Sem) TimedWait(timeout time.Duration) error {
	ts := &C.timespec_t{}
	_, err := C.TimedWait(sem.semt, C.long(timeout/time.Second), C.long(timeout%time.Second), ts)
	return err
}

// Get - return current value of the semaphore.
// If one or more processes or threads are blocked waiting to lock the semaphore with Wait()
// POSIX.1-2001 permits two possibilities for the value returned in sval: either 0 is returned;
// or a negative number whose absolute value is the count of the number of processes
// and threads currently blocked in Wait()
func (sem *Sem) Get() (uint, error) {
	var val C.uint
	_, err := C.Get(sem.semt, &val)
	return uint(val), err
}

// Close - closes the named semaphore, allowing any resources that the system
// has allocated to the calling process for this semaphore to be freed.
func (sem *Sem) Close() error {
	_, err := C.Close(sem.semt)
	if err == nil {
		sem.semt = nil
	}
	return err
